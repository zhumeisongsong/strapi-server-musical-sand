...
- JIRA for project progress and reporting management
...
1. Following branches should be used for primary code management
    ...
    2. `develop` branch can be used for bleeding edge releasable stable code.
    - `staging` branch can be used for to be bleeding edge next-releasable stable code.
    - in case we are doing tag based release, `master` can be used as stable bleeding edge releasable code.

2. `{type}/{JIRAIssueNo}-issue-one-liner` should be the format for branch naming 
    1. see [Type](#__type) section for branch `{type}`s.
    2. Find `{JIRAIssueNo}` in JIRA.

### Pull requests

Pull requests are the only way to propose a value you want to add. Following is a general workflow for submitting any request.

1. Clone the repo and create your branch from `develop`.
2. If you've added code that should be tested
3. Ensure that your code don't failed to build
4. Make pull request to `develop` branch
5. Assign respectives to merge pull request
